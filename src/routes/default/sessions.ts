...
import { ACCESS_FAIL, ACCOUNT_LOGIN_FAIL } from "../../consts.ts";
...
export async function defAccess({ req, res, db }: RouteCallbackProps) {
    ...
    if(!Object.keys(cookies).includes('sid')) return res.status(400).send(generateResponse(false, ACCESS_FAIL));
...
export async function defLevelAccess({ req, res, db }: RouteCallbackProps) {
    ...
    if(!canAccess(user.rank, Number(level))) return res.status(400).send(generateResponse(false, ACCESS_FAIL));
    return res.status(200).send(generateResponse(true, 'success'));
}