...
export const defaultVerify = async (
  hash: string,
  text: string,
): Promise<boolean> => {
...
const DEFAULT_COOKIE_EXPIRY = new Date(
...
export function canAccess(rank: number, required: number): boolean {
  switch (required) {
    ...
    case ranks.ADMIN:
      return rank == ranks.OWNER || rank == ranks.ADMIN;
    case ranks.STREAMER:
      return rank == ranks.OWNER || rank == ranks.STREAMER;
    case ranks.DEFAULT:
      return true;
    default:
      return false;
